...
class HourlyAdapter(private val result2: JsonObject, private val timeZone: String) : RecyclerView.Adapter<HourlyAdapter.ViewHolder>() {
    ...
    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        fun bind(result2: JsonObject, timeZone: String, position: Int) {
            ...
            itemView.findViewById<TextView>(R.id.hourView).text = dateTime(result2.lookup<Int>("hourly.dt")[position + 1], timeZone)
            val uri = result2.lookup<String>("hourly.weather.icon")[position + 1]
            ...
            itemView.findViewById<TextView>(R.id.rainView).text =
                    "${(result2.lookup<Double>("hourly.pop")[position + 1] * 100).roundToInt()}%"
            val temperatureView = itemView.findViewById<TextView>(R.id.temperatureView)
            temperatureView.text = "${result2.lookup<Double>("hourly.temp")[position + 1].roundToInt()}°"
        }
